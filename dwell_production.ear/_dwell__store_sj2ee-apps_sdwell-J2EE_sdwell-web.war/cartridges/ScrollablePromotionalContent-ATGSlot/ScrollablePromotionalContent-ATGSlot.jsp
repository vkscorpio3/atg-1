    ...
    <c:if test="${renderControls eq true}">
      <div class="indicator_wrapper">
        <ol id="carousel_indicator" class="indicator">
          <c:forEach var="promotionalContent" items="${contentItem['atg:contents'].items}" varStatus="status">
            <li data-indicator="${status.count - 1}"><span class="sr_only">${status.count}</span></li>
          </c:forEach>
        </ol>
      </div>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.2/Storefront/j2ee/store.war/cartridges/ScrollablePromotionalContent-ATGSlot/ScrollablePromotionalContent-ATGSlot.jsp#3 $$Change: 1179550 $--%>

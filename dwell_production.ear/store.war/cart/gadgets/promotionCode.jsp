...
<dsp:page>
  ...
  <c:choose>
    <c:when test="${expressCheckout or currentStage == 'confirm'}">
      <%-- 
        Successfully adding/editing the coupon code on the confirmation page 
        will redirect the user to the billing page to respecify thier payment 
        options.
      --%>
      <dsp:input bean="CouponFormHandler.applyCouponSuccessURL" type="hidden" 
                 value="billing.jsp?preFillValues=true&couponCodeChanged=true"/>
      <dsp:input bean="CouponFormHandler.editCouponSuccessURL" type="hidden"  
                 value="billing.jsp?preFillValues=true&couponCodeChanged=true"/>
    </c:when>
    <c:otherwise>
      <dsp:input bean="CouponFormHandler.applyCouponSuccessURL" type="hidden" 
  ...
  <dsp:input bean="CouponFormHandler.editCouponErrorURL"  type="hidden"  
             value="${couponTargetUrl}"/>
  <dsp:input bean="CouponFormHandler.applyCouponErrorURL" type="hidden"  
             value="${couponTargetUrl}"/>
  
</dsp:page>
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.2/Storefront/j2ee/store.war/cart/gadgets/promotionCode.jsp#1 $$Change: 946917 $--%>
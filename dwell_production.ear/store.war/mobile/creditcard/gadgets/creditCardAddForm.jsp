  ...
  <div class="dataContainer">
    <ul class="dataList">
      ...
      <li class="icon-ArrowLeft ${not empty errorMap['creditCardType'] ? 'errorState' : ''}">
        <div class="content">
          ...
          <dsp:select id="paymentInfoAddNewCardCardType" bean="${cardParamsMap}.creditCardType"
            ...
            <dsp:option value="" iclass="default" role="option"><fmt:message key="mobile.creditcard.dropdown.cardType"/></dsp:option>

            <%-- Display supported credit card types as options --%>
            <c:forEach var="creditCardType" items="${supportedCreditCardTypes}">
              <dsp:option value="${creditCardType}" role="option"><fmt:message key="mobile.creditcard.dropdown.${creditCardType}"/></dsp:option>
            </c:forEach>
          </dsp:select>
        </div>
        <c:if test="${not empty errorMap['creditCardType']}">
          <span class="errorMessage">
            <fmt:message key="mobile.common.error.${errorMap['creditCardType']}"/>
          </span>
        </c:if>
      </li>
      ...
      <li ${not empty errorMap['creditCardNumber'] ? 'class="errorState"' : ''}>
        <div class="content">
          <fmt:message var="cardNumberLabel" key="mobile.creditcard.input.placeholder.cardNumber"/>
        ...
        <c:if test="${not empty errorMap['creditCardNumber']}">
          <span class="errorMessage">
            <fmt:message key="mobile.common.error.${errorMap['creditCardNumber']}"/>
          </span>
        </c:if>
      </li>
        ...
        <c:if test="${showSaveCardOption == 'true'}">
          ...
          <li>
            <div class="content">
              ...
              <label for="saveCreditCard" onclick=""><fmt:message key="mobile.creditcard.label.saveCard"/></label>
            </div>
          </li>
        </c:if>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.2/Storefront/j2ee/store.war/mobile/creditcard/gadgets/creditCardAddForm.jsp#1 $$Change: 946917 $--%>

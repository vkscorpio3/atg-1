...
<dsp:page>
    ...
    <c:forEach var="formException" items="${formExceptions}">
      ...
      <c:choose>
        ...
        <c:when test="${errorCode == 'postalCode'}">
          <c:set target="${errorMap}" property="postalCode" value="invalidValue"/>
        </c:when>
        ...
        <c:when test="${errorCode == 'invalidEmailAddress'}">
          <c:set target="${errorMap}" property="email" value="invalidValue"/>
        </c:when>
        <c:when test="${errorCode == 'invalidDateEntered'}">
          <%-- Invalid "Date of Birth" value (entered partially) --%>
          <c:set var="invalidDateEnteredErrorMessage" value="${formException.message}"/>
        </c:when>
      </c:choose>
    </c:forEach>
  </c:if>

  <fmt:message var="pageTitle" key="mobile.myAccount.header.personalInformation"/>
  <crs:mobilePageContainer titleString="${pageTitle}">
    <jsp:body>
      ...
      <%-- ========== Navigation tabs ========== --%>
      <fmt:message var="editProf" key="mobile.myAccount.tab.editProfile"/>
      <dsp:include page="gadgets/myAccountTabBar.jsp">
        <dsp:param name="thirdTabLabel" value="${editProf}"/>
        <dsp:param name="highlight" value="third"/>
      </dsp:include>

      <%-- Display "invalidDateEntered" error message, if any --%>
      <c:if test="${not empty invalidDateEnteredErrorMessage && fn:length(invalidDateEnteredErrorMessage) > 0}">
        <div id="formValidationError">
          <div class="errorMessage">
            <dsp:valueof value="${invalidDateEnteredErrorMessage}" valueishtml="true"/>
          </div>
        </div>
      </c:if>
        ...
        <dsp:form formid="accountProfileEdit" action="${pageContext.request.requestURI}" method="post">
          ...
          <ul class="dataList" role="presentation">
            ...
            <li ${not empty errorMap['firstName'] ? 'class="errorState"' : ''}>
              <fmt:message var="firstNamePlace" key="mobile.myAccount.input.placeholder.firstName"/>
              <div class="content">
              ...
              <c:if test="${not empty errorMap['firstName']}">
                <span class="errorMessage">
                  <fmt:message key="mobile.common.error.${errorMap['firstName']}"/>
                </span>
              </c:if>
            </li>
            <%-- "Last Name" --%>
            <li ${not empty errorMap['lastName'] ? 'class="errorState"' : ''}>
              <fmt:message var="lastNamePlace" key="mobile.myAccount.input.placeholder.lastName"/>
              <div class="content">
              ...
              <c:if test="${not empty errorMap['lastName']}">
                <span class="errorMessage">
                  <fmt:message key="mobile.common.error.${errorMap['lastName']}"/>
                </span>
              </c:if>
            </li>
            <%-- "Email" --%>
            ...
            <li>
              <fmt:message var="postalCodePlace" key="mobile.myAccount.input.placeholder.postalCode"/>
              <%-- "Postal Code" --%>
              <div class="left ${not empty errorMap['postalCode'] ? 'errorState' : ''}">
                ...
                <c:if test="${not empty errorMap['postalCode']}">
                  <span class="errorMessage">
                    <fmt:message key="mobile.common.error.${errorMap['postalCode']}"/>
                  </span>
                </c:if>
              </div>
              <div class="right">
                        ...
                        <c:choose>
                          ...
                          <c:when test="${gender.settableValue == 'unknown'}">
                            <dsp:option iclass="default" value="${gender.settableValue}">
                              <fmt:message key="mobile.myAccount.dropDown.selectGender"/>
                            </dsp:option>
                          </c:when>
            ...
            <li>
              ...
              <div class="left40">
                <div class="content">
                  <label class="hint left"><fmt:message key="mobile.myAccount.label.DOB"/></label>
                </div>
              </div>
              <div class="right60">
                <div class="content">
                  ...
                  <div class="day">
                    <dsp:select id="DOB_DaySelect" bean="RegistrationFormHandler.date" required="false"
                      ...
                      <dsp:option value=""><fmt:message key="mobile.myAccount.dropDown.day"/></dsp:option>
                  ...
                  <div class="year">
                    ...
                    <crs:yearList numberOfYears="100" bean="/atg/store/profile/RegistrationFormHandler.year" id="DOB_YearSelect"
                      ...
                      startYear="${currYear - 100}" iclass="${className}"/>
            ...
            <li>
              <div class="content">
                <div class="checkbox">
                  ...
                  <label for="receive_emails" onclick=""><fmt:message key="mobile.myAccount.label.receiveEmails"/></label>
                </div>
              </div>
            </li>
          </ul>
          ...
          <div class="centralButton">
            <fmt:message var="done" key="mobile.common.button.done"/>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.2/Storefront/j2ee/store.war/mobile/myaccount/accountProfileEdit.jsp#2 $$Change: 953229 $--%>

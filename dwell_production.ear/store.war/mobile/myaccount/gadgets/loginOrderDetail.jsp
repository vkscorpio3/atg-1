<%--
  This page redirects user to the login page if profile is anonymous and attempt to open order detail page.
  This check is needed if user received email with order shipping or order confirmation details
  and click on "Order details" page.
...
<dsp:page>
  ...
  <%-- Retrieve order Id from the SessionBean first. If not found, use input parameter --%>
  ...
  <%-- Set order detail page as success URL after login. Append "orderId" parameter --%>
  <dsp:getvalueof var="orderDetailURL" value="${mobileStorePrefix}/myaccount/orderDetail.jsp?orderId=${orderId}"/>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.2/Storefront/j2ee/store.war/mobile/myaccount/gadgets/loginOrderDetail.jsp#2 $$Change: 953229 $--%>

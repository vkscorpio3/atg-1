...
<dsp:page>
  ...
  <c:choose>
    ...
    <c:otherwise>
      <fmt:message var="pageTitle" key="mobile.checkout.reviewOrder.pageTitle"/>
            ...
            <dsp:form formid="confirm" action="${pageContext.request.requestURI}" method="post">
              ...
              <dsp:droplet name="ProfileSecurityStatus">
                <dsp:oparam name="anonymous">
                  <h2><fmt:message key="mobile.checkout.header.emailConfirmation"/></h2>
                  <div class="roundedBox">
                    <div class="content ${not empty errorMap['email'] ? 'errorState' : ''}">
                      <fmt:message key="mobile.checkout.input.placeholder.email" var="emailHint"/>
                      
                      <%-- Clear email before setting a new one --%>
                    ...
                    <c:if test="${not empty errorMap['email']}">
                      <span class="errorMessage errorMessageConfirmEmail">
              ...
              <div class="cartCheckout">
                <fmt:message var="submitBtnValue" key="mobile.checkout.button.placeOrder"/>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.2/Storefront/j2ee/store.war/mobile/checkout/confirm.jsp#1 $$Change: 946917 $--%>

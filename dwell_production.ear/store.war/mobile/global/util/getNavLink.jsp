...
<dsp:page>
  ...
  <c:choose>
    <c:when test="${navActionObject.getClass().name == 'com.endeca.infront.cartridge.model.UrlAction'}">
      <c:set var="navLink" value="${navActionObject.url}" scope="request"/>
    </c:when>
    <c:when test="${navActionObject.getClass().name == 'com.endeca.infront.cartridge.model.LinkBuilder'}">
      <c:choose>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.2/Storefront/j2ee/store.war/mobile/global/util/getNavLink.jsp#2 $$Change: 953229 $--%>

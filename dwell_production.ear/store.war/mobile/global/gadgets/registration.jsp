        ...
        <c:forEach var="formException" items="${formExceptions}">
          ...
          <c:choose>
            ...
            <c:when test="${errorCode == 'missingRequiredValue'}">
              ...
              <c:set target="${errorMap}" property="${field}" value="mandatoryField"/>
            </c:when>
            ...
            <c:when test="${errorCode == 'atg.droplet.DropletException'}">
              <c:set target="${errorMap}" property="password" value="invalidValue"/>
        ...
        <dsp:form formid="registration" action="${pageContext.request.requestURI}" method="post">
          ...
          <div class="dataContainer">
            ...
            <ul class="dataList">
              ...
              <li ${not empty errorMap['email'] ? 'class="errorState"' : ''}>
                ...
                <c:if test="${not empty errorMap['email']}">
                  <span class="errorMessage"><fmt:message key="mobile.common.error.${errorMap['email']}"/></span>
                </c:if>
              </li>
              ...
              <li ${not empty errorMap['password'] ? 'class="errorState"' : ''}>
                <div class="content">
                  <fmt:message var="createPasswordHint" key="mobile.login.input.placeholder.password"/>
                ...
                <c:if test="${not empty errorMap['password']}">
                  <span class="errorMessage"><fmt:message key="mobile.common.error.${errorMap['password']}"/></span>
                </c:if>
              </li>
              ...
              <li ${not empty errorMap['confirmPassword'] ? 'class="errorState"' : ''}>
                <div class="content">
                  <fmt:message var="createConfirmPasswordHint" key="mobile.myAccount.changePassword.input.placeholder.confirmPassword"/>
                ...
                <c:if test="${not empty errorMap['confirmPassword']}">
                  <span class="errorMessage"><fmt:message key="mobile.common.error.${errorMap['confirmPassword']}"/></span>
                </c:if>
              </li>
              ...
              <li ${not empty errorMap['firstName'] ? 'class="errorState"' : ''}>
                <div class="content">
                  <fmt:message var="firstNameHint" key="mobile.myAccount.input.placeholder.firstName"/>
              ...
              <li ${not empty errorMap['lastName'] ? 'class="errorState"' : ''}>
                <div class="content">
                  <fmt:message var="lastNameHint" key="mobile.myAccount.input.placeholder.lastName"/>
            ...
            <div class="centralButton">
              <fmt:message var="submitBtnValue" key="mobile.login.button.signUp"/>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.2/Storefront/j2ee/store.war/mobile/global/gadgets/registration.jsp#1 $$Change: 946917 $--%>

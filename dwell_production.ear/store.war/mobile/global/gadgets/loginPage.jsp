    ...
    <c:forEach var="formException" items="${formExceptions}">
      ...
      <c:choose>
        <c:when test="${errorCode == 'missingEmail'}">
          <c:set target="${errorMap}" property="email" value="mandatoryField"/>
        </c:when>
        ...
        <c:when test="${errorCode == 'invalidPassword'}">
          <c:set target="${errorMap}" property="badPassword" value="invalidValue"/>
        </c:when>
      </c:choose>
    </c:forEach>
  </c:if>
      ...
      <ul class="dataList" role="presentation">
        <c:if test="${not empty errorMap['email'] || not empty errorMap['badPassword']}">
          <li class="errorStateLine">
            <div class="content">
              <span class="errorMessageLine"><fmt:message key="mobile.common.error.invalidLogin"/></span>
            </div>
          </li>
        </c:if>
        <c:if test="${passwordSent == 'true'}">
          <li class="errorState">
            <span class="errorMessageLine"><fmt:message key="mobile.login.message.passwordSent"/></span>
          </li>
        </c:if>
        <%-- "Email" --%>
        ...
        <li class="${liClassEmail}">
            ...
            <c:if test="${not empty errorMap['login'] || not empty errorMap['emailAddress']}">
              <span class="errorMessage">
                <fmt:message key="mobile.common.error.mandatoryField"/>
              </span>
            </c:if>
        </li>
        <%-- "Password" --%>
        <li ${not empty errorMap['email'] || not empty errorMap['badPassword'] || not empty errorMap['password'] || passwordSent == 'true' ? 'class="errorState"' : ''}>
            <div class="content">
              <fmt:message var="password" key="mobile.login.input.placeholder.password"/>
            ...
            <c:choose>
              <c:when test="${passwordSent == 'true'}">
                <span class="errorMessage normalWidth">
                  <p><fmt:message key="mobile.login.pastePassword"/></p>
                </span>
              </c:when>
        ...
        <c:if test="${not empty errorMap['email'] || not empty errorMap['badPassword']}">
          <li class="errorStateLine icon-ArrowForget">
            <dsp:a page="${mobileStorePrefix}/myaccount/passwordReset.jsp?email=${emailValue}&checkoutLogin=${checkoutLogin}" title="${forgotPasswordTitle}">
              <span class="content">
                <span class="errorMessageLine"><fmt:message key="mobile.login.link.forgotLoginOrPassword"/></span>
              </span>
            </dsp:a>
          </li>
        </c:if>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.2/Storefront/j2ee/store.war/mobile/global/gadgets/loginPage.jsp#2 $$Change: 953229 $--%>

...
<dsp:page>
  ...
  <%-- 
    Define the email subject and title resource keys depending on whether the 
    current order has been fully or partially shipped. 
  --%>
  <c:choose>
    <c:when test="${message.order.stateAsString == 'NO_PENDING_ACTION'}">
      <%-- All items have been shipped. --%>
      <c:set var="subjectResourceKey" 
             value="emailtemplates_orderShipped.subject"/>
      <c:set var="emailTitleKey" 
             value="emailtemplates_orderShipped.title"/>
    </c:when>
    <c:otherwise>
      <%-- Only some items have been shipped. --%>
      <c:set var="subjectResourceKey" 
             value="emailtemplates_orderPartiallyShipped.subject"/>
      <c:set var="emailTitleKey" 
             value="emailtemplates_orderPartiallyShipped.title"/>
    </c:otherwise>
  </c:choose>
  
  <%-- Set the email subject text. --%>
  <fmt:message var="emailSubject" key="${subjectResourceKey}">
    <fmt:param>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.2/Storefront/j2ee/store.war/emailtemplates/orderShipped.jsp#1 $$Change: 946917 $--%>

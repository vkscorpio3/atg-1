<%-- 
  ...
  Optional parameters:
    submitFieldText
      Text on the "commit" field which is added to
      the message shown for missing required data
    errorMessageClass
      CSS class used for <p> tag when rendering error messages       
--%>
<dsp:page>
      ...
      <c:choose>
        <c:when test="${errorCode == 'invalidLoginAlreadyLoggedIn'}">
          <div class="errorMessage">
            ...
            <fmt:message key="myaccount_login.anotherLoginText">
              <fmt:param>
                <dsp:a page="/">
                  <dsp:property bean="/atg/userprofiling/ProfileFormHandler.logoutSuccessURL" value="myaccount/login.jsp"/>
        ...
        <c:when test="${errorCode == 'invalidDate'}">
          <div class="errorMessage">
            ...
            <fmt:formatDate var="formattedExampleDate" 
                            type="date"                                 
                            value="${exampleDate}" 
                            pattern="${dateFormat}"/>
...
<%-- @version $Id: //hosting-blueprint/B2CBlueprint/version/11.2/Storefront/j2ee/store.war/myaccount/gadgets/myAccountErrorMessage.jsp#1 $$Change: 946917 $--%>

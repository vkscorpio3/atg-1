function myFunction(){
 ...
 $("#searchBar").autocomplete({
  ...
  source : function(request, response) {
   url = "/dwellstore/includes/autosuggest.jsp?Ntt=" + $("#searchBar").val() +"*&contentCollection=/content/Shared/Auto-Suggest Panels/Default Auto-Suggest Panel&Dy=1";
     ...
     if (autoSuggestCartridges != null || autoSuggestCartridges.length > 0 && autoSuggestCartridges['@type'] == "DimensionSearchAutoSuggestItem") {
      ...
      for (var i = 0; i < dimSearchGroupList.length; i++) {
       ...
       for (var j = 0; j < dimResultGroup.dimensionSearchValues.length; j++) {
		...
		var $img = $('<img>');
		$img.attr({
      src: '/dwell/javascript/widget/EO.jpg' 
  ...
  select: function( event, ui ) {
   window.location = "/dwellstore/cartridges/DwellProductList/DwellProductList.jsp?Ntt=" + ui.item.value;
  },
  messages: {
         noResults: '',
         results: function() {}
